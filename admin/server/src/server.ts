...
import { priceRoutes } from '#modules/prices/priceRoutes';
import { settingsRoutes } from '#modules/users/settingsRoutes';
...
async function registerRoutes(
  ...
  await server.register(portfolioRoutes, {
    ...
    portfolioService: container.services.portfolio,
  });

  await server.register(aboutRoutes, {
    prefix: '/api/profile',
    aboutService: container.services.about,
    authGuard: container.guards.auth,
  });

  await server.register(priceRoutes, {
    prefix: '/api/price',
    priceService: container.services.prices,
    authGuard: container.guards.auth,
  });

  await server.register(settingsRoutes, {
    prefix: '/api/settings',
    settingsService: container.services.settings,
    authGuard: container.guards.auth,
  });
}
