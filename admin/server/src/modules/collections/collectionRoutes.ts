...
> = (app, { collectionService, authGuard }) => {
  ...
  app.route<{
    ...
    schema: {
      ...
      body: {
        type: 'object',
        required: ['name', 'isActive', 'isLiked', 'slug'],
        properties: {
          name: { type: 'string', minLength: 1 },
          slug: { type: 'string' },
  ...
  app.route<{ Params: { id: string } }>({
    ...
    handler: async (req) => {
      const data = await collectionService.hide(req.params.id);
      return { data };
    },
  });
  ...
  app.route<{ Params: { id: string } }>({
    ...
    handler: async (req) => {
      const data = await collectionService.restore(req.params.id);
      return { data };
    },
  });
